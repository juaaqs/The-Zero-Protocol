<?php
// verify.php
// Handles the 6-digit PIN entry for new user email activation.
...
// 1. Get email from the URL
$email = isset($_GET['email']) ? htmlspecialchars($_GET['email']) : '';

// 2. We NEED the user ID for the resend link
$verification_user_id = $_SESSION['temp_verification_user_id'] ?? null;

// 3. Fallback: If we have the ID but not the email (e.g., after an error/resend)
if (empty($email) && $verification_user_id) {
    require_once 'db_config.php';
    try {
        $stmt = $pdo->prepare("SELECT email FROM users WHERE user_id = ?");
        $stmt->execute([$verification_user_id]);
...
<head>
    ...
    <title>The Zero Protocol - Email Activation</title>
    ...
    <style>
        ...
        /* --- VERIFY DOSSIER --- */
        .dossier-wrapper {
            width: 100%; height: 100%; display: flex; justify-content: center; align-items: center; position: fixed; top: 0; left: 0; z-index: 4;
        }
        .dossier-container {
            position: relative; z-index: 5; width: 420px; padding: 30px 40px; background-color: #f5eeda; 
            border: 1px solid #c9b79c; box-shadow: 10px 10px 25px rgba(0, 0, 0, 0.5);
        }
        .dossier-title {
            font-family: 'Special Elite', monospace; font-size: 2.5em; color: #2F2F2F; text-align: center; margin: 0 0 15px 0; text-transform: uppercase;
        }
        .verify-notice {
            font-family: 'Merriweather', serif; font-size: 1em; color: #333; text-align: center; margin-bottom: 25px; line-height: 1.5;
        }
        .pin-group {
            display: flex; justify-content: center; gap: 10px; margin-bottom: 25px;
        }
        .pin-input {
            width: 50px; height: 60px; font-family: 'Special Elite', monospace;
            font-size: 2em; text-align: center; border: none;
            border-bottom: 3px solid #aaa; background-color: rgba(210, 180, 140, 0.2);
            transition: border-color 0.3s;
        }
        .pin-input:focus {
            outline: none; border-bottom-color: #b71c1c;
        }
        .btn {
        ...
        .message {
            font-family: 'Special Elite', monospace; font-size: 1.1em; text-align: center; margin-bottom: 15px; border: 1px dashed; padding: 10px; background-color: rgba(0,0,0, 0.05);
        }
        .message.error {
...
<body>
    ...
    <div id="intro-screen" class="fullscreen-container">
        <div style="text-align: center;">
            <div class="intro-text">ACCESS KEY CONFIRMATION</div>
            <button class="btn-proceed" id="intro-continue-btn">[ Initiate Verification ]</button>
        </div>
    </div>
    ...
    <div id="main-content" class="fullscreen-container">
        ...
        <div class="dossier-wrapper">
            <div class="dossier-container">
                ...
                <h1 class="dossier-title">Email Activation</h1>
                
                <p class="verify-notice">
                    A 6-digit access key has been dispatched to <strong><?php echo $email; ?></strong>. Enter the code below to finalize your agent registration.
                </p>
                
                <?php
                    // This now checks the unified session message
                ...
                <div class="dossier-footer">
                    <a href="auth_controller.php?action=resend_code">Resend Key</a> | <a href="signup.php">Entered Wrong Email?</a>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            if (introButton && introScreen.style.display !== 'none') {
                ...
                // If intro is hidden (due to error/success redirect), focus pin box immediately
                document.querySelector('.pin-input').focus();
                // And play audio on the next click
