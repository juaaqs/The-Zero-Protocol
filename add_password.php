...
if ($hasPassword) {
    header("Location: account_settings.php?error=Password login already enabled."); exit;
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Add Password Login - Defuse It!</title>
...
<body class="aesthetic-bg">
    ...
    <div class="auth-container-cartoon">
        <h1>Add Password Login</h1>
        <p>Create a password to enable logging in with your email address as an alternative to Google.</p>
        <?php
        if (isset($_GET['error'])) {
        ...
        ?>
        <form action="handle_add_password.php" method="POST">
            ...
            <button type="submit" class="btn btn-rounded">Set Password</button>
        </form>
        <p class="auth-link">
